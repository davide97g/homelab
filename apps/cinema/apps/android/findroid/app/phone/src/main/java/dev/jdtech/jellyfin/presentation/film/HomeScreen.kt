...
import it.davideghiotto.cinema.home.FeatureBand
import it.davideghiotto.cinema.home.featureItems
import it.davideghiotto.cinema.home.rememberPlayLauncher
...
private fun HomeScreenLayout(
    ...
    onPlay: (FindroidItem) -> Unit = rememberPlayLauncher(),
) {
    ...
    Box(modifier = Modifier.fillMaxSize().semantics { isTraversalGroup = true }) {
        PullToRefreshBox(isRefreshing = false, onRefresh = { onAction(HomeAction.OnRetryClick) }) {
            LazyColumn(
                ...
                // Cinema: the home screen opens on one large paging card, the
                // way apps/web does, rather than on upstream's suggestions
                // carousel. It hides itself when there is nothing behind it.
                featureItems(state)
                    .takeIf { it.isNotEmpty() }
                    ?.let { items ->
                        item(key = "cinema-feature") {
                            FeatureBand(
                                items = items,
                                itemsPadding = itemsPadding,
                                onPlay = onPlay,
                                onDetails = { onAction(HomeAction.OnItemClick(it)) },
                            )
                        }
                    }
