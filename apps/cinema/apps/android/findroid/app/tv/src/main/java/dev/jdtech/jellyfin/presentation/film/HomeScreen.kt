...
private fun HomeScreenLayout(state: HomeState, onAction: (HomeAction) -> Unit) {
    ...
    LazyColumn(
        ...
        // Cinema: one large card leads the screen, the way apps/web and the
        // phone app do, rather than upstream's suggestions carousel. It hides
        // itself when there is nothing behind it.
        featureItems(state)
            ...
            ?.let { items ->
                item(key = "cinema-feature") {
                    FeatureBand(
                        items = items,
                        onSelect = { onAction(HomeAction.OnItemClick(it)) },
