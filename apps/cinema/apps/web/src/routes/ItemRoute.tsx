...
export function ItemRoute() {
  ...
  return (
    <article>
      <div className="relative h-[45vh] min-h-[300px] w-full overflow-hidden rounded-3xl shadow-panel">
        {backdrop && <img src={backdrop} alt="" className="size-full object-cover object-top" />}
        <div className="absolute inset-0 bg-gradient-to-t from-canvas-deep via-canvas-deep/60 to-transparent" />
        <div className="absolute inset-0 rounded-3xl border border-[var(--hairline)]" />
      </div>

      <div className="relative -mt-28 flex flex-col gap-8 px-4 md:flex-row md:px-8">
        {poster && (
          <img
            src={poster}
            alt={item.Name ?? ''}
            className="w-44 shrink-0 self-start rounded-2xl border border-[var(--hairline)] shadow-panel"
          />
        )}
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {offline ? (
              ...
              <>
                ...
                <QuackButton asChild size="lg" variant="outline" className="rounded-pill">
