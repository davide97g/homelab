...
final class RootCoordinator: ObservableObject {
    ...
    private var selectedAccentColor: Color = .cinemaPrimary
    ...
    private func setAppDefaultsObservation() {
        ...
        accentColorCancellable = Task {
            applyAccentColor(.cinemaPrimary)
        }
        .asAnyCancellable()
        ...
        appearanceCancellable = Task {
            ...
            // Cinema is dark-only, so a change to the stored preference still
            // resolves to dark rather than to the value that was picked.
            for await _ in Defaults.updates(.appAppearance) {
    ...
    /// Cinema forces dark.
    ///
    /// Reel has no light values -- following the system renders black text on a
    /// white page -- and changing only the stored default would not help anyone
    /// whose preference was already written to disk.
    ///
    /// This is not the same job as `preferredColorScheme(.dark)` in
    /// `SwiftfinApp`, and neither one covers for the other: that one is the
    /// SwiftUI hierarchy from the first frame, this one is
    /// `UIWindow.overrideUserInterfaceStyle` -- what UIKit-hosted screens read
    /// -- and it is also what writes `Defaults[.appearance]`, which
    /// `applyAccentColor` reads to decide which way to mix for contrast.
    @MainActor
