...
struct DefaultContentGroupProvider: ContentGroupProvider {
    ...
    private func _makeGroups(userViews: [BaseItemDto]) -> [any ContentGroup] {
        ...
        #else
        // Cinema: the feature band, then the resume row underneath it -- the
        // same shape as apps/web's home screen. One view model between them, so
        // the screen asks for /Items/Resume once rather than twice. tvOS keeps
        // its own cinematic group above.
        let featureContentGroup = FeatureContentGroup(
            resumeLibrary: ResumeItemsLibrary(mediaTypes: [.video]),
            recentlyAddedLibrary: RecentlyAddedLibrary()
        )
        ...
        ResumeRowContentGroup(
            viewModel: featureContentGroup.viewModel
        )
        #endif
