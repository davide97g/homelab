    ...
    struct PlaybackProgress: View {
        ...
        private var capsuleSlider: some View {
            AlternateLayoutView {
                ...
                // Cinema: progress is the one action colour. The track and
                // the buffered span stay neutral, or the whole bar goes red --
                // SwiftUI derives secondary and tertiary from primary when only
                // primary is given.
                .foregroundStyle(
                    manager.state == .loadingItem ? AnyShapeStyle(.gray) : AnyShapeStyle(Color.cinemaPrimary),
                    Color.cinemaForeground,
                    Color.cinemaForeground.opacity(0.55)
                )
            }
