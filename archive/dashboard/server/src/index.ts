import { serve } from "@hono/node-server";
import { Hono } from "hono";
import { readFile, stat } from "node:fs/promises";
import { dirname, extname, join, normalize, resolve, sep } from "node:path";
import { fileURLToPath } from "node:url";
import { checkPassword, cookieHeader, issue, readCookie, verify } from "./auth.js";
import { config } from "./config.js";
import { overview } from "./overview.js";

const here = dirname(fileURLToPath(import.meta.url));
// dist/index.js sits next to the built frontend, which the Dockerfile copies in.
const staticRoot = join(here, "public");

if (!config.password) {
  console.error("DASHBOARD_PASSWORD is not set — refusing to start an unprotected dashboard.");
  process.exit(1);
}
if (!config.sessionSecret) {
  console.warn("SESSION_SECRET is not set — a random one was generated, so a restart logs you out.");
}

const app = new Hono();

/** Brute force is the only realistic attack on a single-password login, so the
 *  attempt rate is capped per address. Small and in-memory on purpose: one
 *  process, one user, no need for anything shared. */
const attempts = new Map<string, { count: number; first: number }>();
const WINDOW_MS = 5 * 60_000;
const MAX_ATTEMPTS = 10;

function throttled(ip: string): boolean {
  const now = Date.now();
  const entry = attempts.get(ip);
  if (!entry || now - entry.first > WINDOW_MS) {
    attempts.set(ip, { count: 1, first: now });
    return false;
  }
  entry.count += 1;
  return entry.count > MAX_ATTEMPTS;
}

function clientIp(c: { req: { header: (k: string) => string | undefined } }): string {
  return (
    c.req.header("cf-connecting-ip") ??
    c.req.header("x-forwarded-for")?.split(",")[0]?.trim() ??
    "local"
  );
}

app.post("/api/login", async (c) => {
  const ip = clientIp(c);
  if (throttled(ip)) return c.json({ error: "too many attempts, wait a few minutes" }, 429);

  const body = await c.req.json<{ password?: string }>().catch(() => ({}) as { password?: string });
  if (!checkPassword(body.password ?? "")) {
    return c.json({ error: "wrong password" }, 401);
  }
  attempts.delete(ip);
  c.header("Set-Cookie", cookieHeader(issue()));
  return c.json({ ok: true });
});

app.post("/api/logout", (c) => {
  c.header("Set-Cookie", cookieHeader(null));
  return c.json({ ok: true });
});

app.get("/api/session", (c) => c.json({ authenticated: verify(readCookie(c.req.header("cookie"))) }));

app.use("/api/overview", async (c, next) => {
  if (!verify(readCookie(c.req.header("cookie")))) return c.json({ error: "unauthorized" }, 401);
  await next();
});

app.get("/api/overview", async (c) => {
  const data = await overview();
  // Every number here is a few seconds old by design; never let a proxy or the
  // browser serve an older one on top of that.
  c.header("Cache-Control", "no-store");
  return c.json(data);
});

app.get("/healthz", (c) => c.text("ok"));

const MIME: Record<string, string> = {
  ".html": "text/html; charset=utf-8",
  ".js": "text/javascript; charset=utf-8",
  ".css": "text/css; charset=utf-8",
  ".json": "application/json; charset=utf-8",
  ".svg": "image/svg+xml",
  ".png": "image/png",
  ".webp": "image/webp",
  ".ico": "image/x-icon",
  ".woff2": "font/woff2",
};

/** Serve the built frontend, and fall back to index.html for anything that is
 *  not a real file -- the app is a single page, every route is the same shell.
 *  Login state lives in a cookie the client checks, so handing the shell to a
 *  stranger reveals nothing; the data routes are what is guarded. */
app.get("*", async (c) => {
  const requested = decodeURIComponent(new URL(c.req.url).pathname);
  const candidate = resolve(staticRoot, `.${normalize(requested)}`);
  const inRoot = candidate === staticRoot || candidate.startsWith(staticRoot + sep);

  if (inRoot && requested !== "/") {
    try {
      if ((await stat(candidate)).isFile()) {
        const type = MIME[extname(candidate)] ?? "application/octet-stream";
        // Vite fingerprints everything under /assets, so those are immutable.
        const immutable = requested.startsWith("/assets/");
        c.header("Content-Type", type);
        c.header("Cache-Control", immutable ? "public, max-age=31536000, immutable" : "no-cache");
        return c.body(await readFile(candidate));
      }
    } catch {
      // fall through to the shell
    }
  }

  try {
    c.header("Cache-Control", "no-cache");
    return c.html(await readFile(join(staticRoot, "index.html"), "utf8"));
  } catch {
    return c.text("frontend not built — run `pnpm build` in frontend/", 500);
  }
});

serve({ fetch: app.fetch, port: config.port, hostname: "0.0.0.0" }, (info) => {
  console.log(`mediarr dashboard listening on :${info.port}`);
});
